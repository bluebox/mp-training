...
import com.library.interfaces.IssueRecordDaoInterface;

@Repository
public class IssueRecordDao implements IssueRecordDaoInterface{
	...
	public boolean issueBook(IssueRecord issueRecord) {
		...
		try {
			...
			if (!alreadyIssued(issueRecord.getBookId(), issueRecord.getMemberId())) {
	...
	public boolean returnBook(IssueRecord issueRecord) {
		try {
			...
			LocalDate issueDate = jdbcTemplate.queryForObject(sql,
					(rs, rowNum) -> rs.getDate("issueDate").toLocalDate(), bookId, memberId,
			...
			int rowsUpdated = jdbcTemplate.update(sqlUpdate, RecordStatus.RETURNED.getCode(),
					Date.valueOf(LocalDate.now()), issueRecord.getBookId(), issueRecord.getMemberId(),
					RecordStatus.ISSUED.getCode());
			...
			int logRows = jdbcTemplate.update(sqlLog, issueRecord.getBookId(), issueRecord.getMemberId(),
					RecordStatus.RETURNED.getCode(), Date.valueOf(issueDate), Date.valueOf(LocalDate.now()));
