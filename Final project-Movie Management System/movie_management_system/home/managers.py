...
class HallManager:
    ...
    @staticmethod
    def get_data_filtered_by_hallandDate(id, date,mid):
        'gets records from datewise hall'
        hall = DatewiseHall.objects.filter(Hall_id=id, Date=date,Movie_id=mid).first()
        return hall

    @staticmethod
    def hallFilteredByDate(id, date,stime):
        hall = DatewiseHall.objects.filter(Hall_id=id, Date=date,startTime=stime).first()
