...
def test_classes_are_object_factories():
    ...
    assert True == (q1 is Queue)
    assert True  == (q2 is Queue)
    assert False == (q2 is q1)

    assert True == isinstance(q1, Queue)
    assert True== isinstance(q2, Queue)

    assert 12 == len(dir(Queue))
    assert 12 == len(dir(q1))
    assert 12 == len(dir(q2))
...
def test_classes_init_constructor():
    ...
    assert False == (self_argument is q1)
...
# """<basics.test_20_underestanding_classes.test_classes_init_construc...""""
    # assert False == Queue.push.__ == q1_push.__self__      #bound method
    #    assert ==q1_push.__self__
...
-
"""
# C:\Users\ssred\training-project\mp-training\Tasks\pybasics\basics\test_20_underestanding_classes.py
