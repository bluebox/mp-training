...
def test_function_call_with_keyword_arguments():
    ...
    assert "TypeError: demo() missing 1 required positional argument: 'first'" == demo(second=20)
    assert "TypeError: demo() missing 1 required positional argument: 'first'" == demo(second=20, third=30)
...
def test_function_with_keyword_args():
    ...
    assert {'age':10,'height':100} == demo_with_keyword_args("jack", address="address", age=10, height=100)
