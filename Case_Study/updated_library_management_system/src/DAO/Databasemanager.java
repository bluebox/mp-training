...
public class Databasemanager implements bookInterface {
	...
	public boolean updateBookDetails(int id,Book book) {
		...
		try {
			...
			preparedstatementlog.setInt(1,id);

			preparedstatementlog.setString(2,book.getTitle());
			preparedstatementlog.setString(3,book.getAuthor());
			preparedstatementlog.setString(4,book.getCategory());
			preparedstatementlog.setString(5,book.getStatus().getType());
			if(book.getStatus()==Status.INACTIVE) {
				preparedstatementlog.setString(6,null);
				Updatebookavailability(book,null);
				
			}
			else {
			preparedstatementlog.setString(6,book.getAvailability().getType());
			}
	...
	public Book getBookwithId(int bookid) {
		...
		try {
			...
			ResultSet res=preparedstatement.executeQuery();
			if(res.next()) {
				book=new Book(res.getString("title"),res.getString("author"),res.getString("category"),Status.getstatus(res.getString("status")),Availability.getstatus(res.getString("availability")));
			}
