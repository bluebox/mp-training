...
public class SecurityConfiguration {
	    ...
	    SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http) throws Exception {
		 ...
		 .authorizeHttpRequests(requests->requests.requestMatchers("/").authenticated()
				 ...
				 .logout(lc->lc.invalidateHttpSession(true).logoutSuccessUrl("/").permitAll());
	    ...
	    public CorsConfigurationSource corsConfigurationSource() {
	        ...
	        configuration.setAllowedOrigins(Arrays.asList("http://localhost:3000/")); // Specify your single origin
