  ...
  fetch('http://192.168.0.73:32114/partner/get-states?countryCode=IN')
    ...
    .then(states => {
      ...
      states.forEach(state => {
        ...
        option.textContent = state.name;  //showing the corresponding state name
...
function loadCities(stateCode) {
  fetch(`http://192.168.0.73:32114/partner/get-cities-for-state?stateCode=${stateCode}`)
    ...
    .then(cities => {
      ...
      cities.forEach(city => {
        const option = document.createElement('option');
        option.value = city.code;        // Usage of the city code for value
        option.textContent = city.name;  // Showing the corresponding city name
