...
public class Analytis {
    ...
    public static void main(String[] args) {
        try {
            ...
            // CSV export
            exportToCSV(topTasksPerDept, bugFixGrouped, avgHoursPerEmployee, deptSwitches, projectSwitches, employeeIdToName, result  );
    ...
    private static void exportToCSV(
        ...
        // 1. Top Tasks Per Dept
        try (PrintWriter writer = new PrintWriter("reports/Top_Tasks_Per_Dept.csv")) {
            writer.println("Department,Task Category,Hours Worked");
            for (Map.Entry<String, List<EmployeeWorkLog>> entry : topTasks.entrySet()) {
                for (EmployeeWorkLog log : entry.getValue()) {
                    writer.println(String.join(",",
                            escapeCsv(log.getDepartment()),
                            escapeCsv(log.getTaskCategory()),
                            String.valueOf(log.getHoursWorked())));
                }
            }
            System.out.println("CSV report generated: reports/Top_Tasks_Per_Dept.csv");
        }

        // 2. Bug Fix Grouped
        try (PrintWriter writer = new PrintWriter("reports/Bug_Fix_Grouped.csv")) {
            writer.println("Category,Day of Week,Total Hours");
            for (String cat : bugFix.keySet()) {
                for (String day : bugFix.get(cat).keySet()) {
                    writer.println(String.join(",",
                            escapeCsv(cat),
                            escapeCsv(day),
                            String.valueOf(bugFix.get(cat).get(day))));
                }
            }
            System.out.println("CSV report generated: reports/Bug_Fix_Grouped.csv");
        }

        // 3. Avg Daily Hours
        try (PrintWriter writer = new PrintWriter("reports/Avg_Daily_Hours.csv")) {
            writer.println("Employee ID,Average Hours");
            for (Map.Entry<String, Double> e : avgHours.entrySet()) {
                writer.println(String.join(",",
                        escapeCsv(e.getKey()),
                        String.valueOf(e.getValue())));
            }
            System.out.println("CSV report generated: reports/Avg_Daily_Hours.csv");
        }

        // 4. Dept Switches
        try (PrintWriter writer = new PrintWriter("reports/Dept_Switches.csv")) {
            writer.println("Month,Switch Count");
            for (Map.Entry<String, Long> e : deptSwitch.entrySet()) {
                writer.println(String.join(",",
                        escapeCsv(e.getKey()),
                        String.valueOf(e.getValue())));
            }
            System.out.println("CSV report generated: reports/Dept_Switches.csv");
        }

        // 5. Project Switches
        try (PrintWriter writer = new PrintWriter("reports/Project_Switches.csv")) {
            writer.println("Month,Switch Count");
            ...
            System.out.println("CSV report generated: reports/Project_Switches.csv");
        }

        // 6. Frequent Project Changers
        try (PrintWriter writer = new PrintWriter("reports/Frequent_Project_Changers.csv")) {
            writer.println("Employee ID,Employee Name");
            for (String empId : result) {
                writer.println(String.join(",",
                        escapeCsv(empId),
                        escapeCsv(employeeIdToName.get(empId))));
            }
            System.out.println("CSV report generated: reports/Frequent_Project_Changers.csv");
        }
    }
