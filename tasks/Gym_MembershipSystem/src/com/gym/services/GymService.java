...
public class GymService {
    ...
    public void assignPlan(int id, String plan, int durationInMonths) {
        ...
        if (member.membershipPlan != null) {
        	double fees = durationInMonths * MembershipPlan.Plan.valueOf(plan).getMonthlyFee();
            fees =  ( durationInMonths>=3 && durationInMonths<6) ? 0.95*fees : (durationInMonths>=6) ? 0.9*fees : fees;
    ...
    public void updatePersonalDetails(Member member) {
        try {
            ...
            if(member.getName().equalsIgnoreCase(name) && member.getAge()==age) {
            	System.out.println("Details are same as before ");
            }
            else {
