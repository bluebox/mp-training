  ...
  constructor(
    ...
    this.routeSubscription = this.route.params.subscribe((res) => {
      if (parseInt(res['id'])) {
        ...
        this.getBookingSubscription = this.dataservice
          .getBooking(parseInt(res['id']))
          .subscribe(
            (data) => {
              ...
              this.BookingForm.get('no_of_people')?.setValue(
                bookingObj.no_of_people
              );
              this.BookingForm.get('passenger_details')?.setValue(
                bookingObj.passenger_details
              );
              this.BookingForm.get('isCancelled')?.setValue(
                bookingObj.isCancelled
              );
            },
            (err) => {
              if (err.status == 404) {
  ...
  addBookingObj() {
    if (this.id) {
      if (this.BookingForm.valid) {
        this.editBookingSubscription = this.dataservice
          .editBooking(this.BookingForm.value, this.id)
          .subscribe(
            (data) => {
              console.log(data);
      ...
      if (this.BookingForm.valid) {
        this.addBookingSubscription = this.dataservice
