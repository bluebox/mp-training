...
const RoleManagement = ({ username , onNavigate}) => {
	  ...
	  useEffect(() => {
	   ...
	   const fetchroles = async () => {
	     try {
	       ...
	       const assignedResponse = await fetch(`/role/getUserAssignedRoles/${username}`);
		   
		   fetch('/role/getroles').then(res => res.json()).then(console.log)
		   fetch(`/role/getUserAssignedRoles/${username}`).then(res => res.json()).then(console.log)
		  ...
		  const assignedRoleIds = new Set(
		    assignedRolesData.flatMap(r => r.roleId)
		  );
  ...
  const handleSubmit = async(e) => {
	...
	if(payload === null || payload.roleId.length === 0){
...
*/	  }
		...
		setMessage(`Roles Assigned to ${username}`);
