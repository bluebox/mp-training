...
def test_truth_values():
    ...
    assert not None == truth_test(0, "0")
    assert not None == truth_test(1, "1")
    assert not None == truth_test("a", "non-empty-string")
    assert not None == truth_test((1,2), "non-empty-tuple")
    assert not None == truth_test([1], "non-empty-list")
    assert not None == truth_test({1:2}, "non-empty-dict")
    assert not None == truth_test({1}, "non-empty-set")
