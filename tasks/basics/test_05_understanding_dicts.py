"""MODULE TO INCREASE FAMILIARITY WITH DICTIONARY"""
__author__ = 'Hari'
...
def test_dictionary_is_indexed_by_key():
    """test dictionary is indexed by key"""
    word_to_digit = {"one": 1, "two": 2}
    ...
    try:
        ...
        # Note that numeric indices don't mean much like in case of lists and tuples
        print(ex)   # ex=1 (value from dict key-value pair stored in exception)
        assert True


def test_dictionary_is_mutable():
    """test dictionary is mutable"""
    word_to_digit = { "one" : 1, "two" : 2}
...
def test_dictionary_keys_and_values():
    """test for dictionary keys and values"""
    word_to_digit = { "one" : 1, "two" : 2}
...
THREE_THINGS_I_LEARNTS = """
-"in" keyword can't find values by dictionary name. (dict.values() need to specified)
-dictionary don't take mutable objects as key (not even tuple with list{mutabe} as an element in it is allowed)
-dictionary can take mutable objects as values.
"""
