...
def test_lc_map_func():
    """learning comprehensions"""
    input_no = [1, 2, 3]
    result = [square(x) for x in input_no]
    assert [1, 4, 9] == result
...
def test_lc_filter_func():
    """learning comprehensions"""
    input_no = range(10)
    result = [x for x in input_no if is_even(x)]
...
def test_sc_filtering():
    """learning comprehensions"""
    all_no = set(range(10))
    evens = {x for x in all_no if x % 2 == 0}
    assert {0, 2, 4, 6, 8} == evens

    odds = {x for x in all_no if x % 2 == 1}
    assert {1, 3, 5, 7, 9} == odds


# three_things_i_learnt = """
# about basics of how to test our code using assert
# about list, dictionary and set comprehension
