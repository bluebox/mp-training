...
def test_function_type():
    """check for type"""
    assert "fucntion" == type(my_print).__name__
    assert "fucntion" == type(my_increment).__name__
    assert "fucntion" == type(test_function_type).__name__
...
def test_function_defaults_are_evaluated_at_definition_time():
    """function"""
    assert ['h', 'i'] is my_add_to_list1("hi")
    assert ['h','i','b','y','e'] is my_add_to_list1("bye")
...
-function can be passed as an object to another function
