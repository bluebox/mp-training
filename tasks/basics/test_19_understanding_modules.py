...
from tasks.basics import module1, module2
...
def test_module_import_affects_current_namespace():
    ...
    def inner_func():
        ...
        return tasks.basics.module1.greet("jack")

    assert "module1 says hi to jack" == module1.greet("jack")
    assert "module1 says hi to jack" == inner_func()
    ...
    assert False is ('module2' in locals())
    assert True is ('module2' in globals())
...
s1 = set(dir())
from tasks.basics.module3 import _m3_func3, m3_func1, m3_func2
s2 = set(dir())
from tasks.basics.module4 import _m4_func3, m4_func1, m4_func2
s3 = set(dir())

#def test_module_star_import():
    # * imports are not allowed within functions, so we had to do it at global scope
    #assert {'m3_func3', 'm3_func2'} == (s2 - s1)  # what did module3 import bring in.
