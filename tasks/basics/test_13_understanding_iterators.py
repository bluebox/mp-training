...
def test_enumerate_iter():
    ...
    try:
        ...
        assert StopIteration == list_iter.next() #note what happens when items are finished.
...
def test_find_builtins_that_work_on_iterables():
    """function"""
    funcs = ["reduce", "map", "frozenset","list", "tuple", "dict", "set","all","sum","sorted","any","max","min"]
    assert 13 == len(funcs)


THREE_THNGS_I_LEARNT= """
-.join() can only be used with string
-when dict is sorted, it is sorted according to the keys
-iterator returns StopIteration after end of iterables
"""
