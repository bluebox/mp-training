"""MODULE FOR UNDERSTANDING ITERATORS"""
__author__ = 'Hari'
...
def test_iterator_type():
    """test iterator type PYTHON-2"""
    list_iter = iter(["one", "two", "three"])
    assert 'listiterator' == type(list_iter).__name__
    assert True is hasattr(list_iter, "next")

    string_iter = iter("hello")
    assert 'striterator' == type(string_iter).__name__
    assert True is hasattr(string_iter, "next")
...
def test_functions_that_work_on_iterables():
    """test functions that work on iterables"""
...
def test_find_builtins_that_work_on_iterables():
    """test find builtins that work iterables"""
    funcs = ['list', 'tuple', 'dict', 'set', 'str']
    assert 5 == len(funcs)


THREE_THINGS_I_LEARNT = """
-in python2 iterator objects had next() as an attribute
-join function does not work for list of integers
-list, tuple, dict, set and string are iterables
"""
