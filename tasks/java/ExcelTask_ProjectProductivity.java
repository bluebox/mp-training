...
public class ExcelTask_ProjectProductivity {
    ...
    static ArrayList<EmployeeWorkLog> readExcel() throws Exception {
        String input = "C:\\Users\\mourya\\OneDrive\\Desktop\\Sample_Employee_WorkLogs.xlsx";
    ...
    static HashMap<String, ProjectStats> processData(ArrayList<EmployeeWorkLog> workers) {
        HashMap<String, ProjectStats> projData = new HashMap<>();

        for (EmployeeWorkLog worker : workers) {
            String proj = worker.getProjectId();
            if (!projData.containsKey(proj)) {
                projData.put(proj, new ProjectStats(proj));
            }
            ProjectStats stats = projData.get(proj);
            stats.totalHours = stats.totalHours + worker.getHoursWorked();
            stats.employeeIds.add(worker.getEmployeeId());
        }

        for (ProjectStats stats : projData.values()) {
            if (stats.employeeIds.size() > 0) {
    ...
    static void makeExcel(HashMap<String, ProjectStats> projects) {
        try {
            String output = "C:\\Users\\mourya\\OneDrive\\Desktop\\Project_Productivity_Output.xlsx";
