...
public class IssueInsuranceRepo implements IssueInsuranceInterface {
    ...
    public List<IssueInsuranceModel> viewallinsurences() {
        String viewsql = "select IssueInsuranceId, InsuranceId, UserId, StartDate, EndDate, EstimationDate, AmountPaid, RemainingAmount, IssueStatus, PaymentType, AdminName, AdminId, ChasisNumber, RegistrationNumber, ModifiedbyId, ModifiedDate, remainingperiod, UpgradeRequestDate FROM issueinsurance";
        List<IssueInsuranceModel> viewallissue = template.query(viewsql, new IssueinsuranceRowMapper());
        return viewallissue.isEmpty() ? Collections.emptyList() : viewallissue;
    }
    ...
    public boolean Upgradeissueinsurance(IssueInsuranceModel issue, int InsuranceId) {
        if ("A".equals(issue.getIssueStatus().getType())) {
            ...
            String updateSql = "update issueinsurance set InsuranceId=?, AmountPaid=?, RemainingAmount=?, ModifiedDate=?, remainingperiod=? WHERE IssueInsuranceId=?";
            int result = template.update(updateSql, InsuranceId, issue.getAmountPaid(), issue.getRemainingAmount(), issue.getModifiedDate(), issue.getRemainingperiod(), issue.getIssueInsuranceId());
            return result > 0;
        } else {
            String insertLog = "insert into issueinsurance_log (IssueInsuranceId, InsuranceId, UserId, StartDate, EndDate, EstimationDate, AmountPaid, RemainingAmount, IssueStatus, PaymentType, AdminName, AdminId, ChasisNumber, RegistrationNumber, ModifiedbyId, ModifiedDate, remainingperiod, UpgradeRequestDate) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public IssueInsuranceModel getIssueInsuranceById(int IssueInsuranceId) {
        String query = "select IssueInsuranceId, InsuranceId, UserId, StartDate, EndDate, EstimationDate, AmountPaid, RemainingAmount, IssueStatus, PaymentType, AdminName, AdminId, ChasisNumber, RegistrationNumber, ModifiedbyId, ModifiedDate, remainingperiod, UpgradeRequestDate FROM issueinsurance WHERE IssueInsuranceId=?";
    ...
    public IssueInsuranceModel getUniquebyids(int adminId,int insuranceId,int userId) {
    	String query="select IssueInsuranceId, InsuranceId,UserId,StartDate,StartDate,EstimationDate,AmountPaid,RemainingAmount,IssueStatus,PaymentType,AdminName,AdminId,ChasisNumber,RegistrationNumber,ModifiedbyId,ModifiedDate,remainingperiod,UpgradeRequestDate from issueinsurance where adminId=? and insuranceId=? and userId=? and IssueStatus'N' ";
