...
export class CustomerProfileComponent implements OnInit {
  ...
  constructor(private service : GeneralService, private route: Router) {
    ...
    this.service.getprofileDetails().subscribe(data=>{
      ...
      if(this.customerData){
       this.customer_group.get('name')?.setValue(this.customerData.name);
       this.customer_group.get('password')?.setValue(this.customerData.password);
       this.customer_group.get('email')?.setValue(this.customerData.email);
       this.customer_group.get('dl_no')?.setValue(this.customerData.dl_no);
       this.customer_group.get('contact_no')?.setValue(this.customerData.contact_no);
       this.customer_group.get('address')?.setValue(this.customerData.address);
      }
    })
    // this.getCustomer()
  ...
  updateCustomerDetails(){
    ...
    this.service.updateCustomerProfile(this.customer_group.value).subscribe((data : any) =>{
      ...
      window.sessionStorage.setItem('customer_id', JSON.stringify(data)), this.route.navigate(['c-profile']) } )
  }
