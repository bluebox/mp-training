...
public class MemberController {
    ...
    @PostMapping("/add")
    public boolean addMember(@RequestBody Member member) throws Exception {
    	try {
    		return memberService.addMember(member);
		} catch (Exception e) {
			if(e.getCause() instanceof SQLIntegrityConstraintViolationException)
			throw new Exception("Member already exists. Please check your details");
		}
		return false;
    }
    ...
    public boolean updateMember(@PathVariable Integer memberId, @RequestBody Member member) {
    ...
    public boolean deleteMember(@PathVariable Integer memberId) throws Exception {
    	try {
    		return memberService.deleteMembers(Arrays.asList(memberId));
    	} catch (Exception e) {
			if(e.getCause() instanceof SQLIntegrityConstraintViolationException)
			throw new Exception("Member is issued with a book so can't deleted.");
		}
		return false;
    }
    ...
    public List<Map<String, String>> getGenders() {
        return Arrays.stream(Gender.values())
                .map(gender -> Map.of("displayName", gender.name(), "code", String.valueOf(gender.getCode())))
