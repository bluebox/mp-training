	...
	private void initialize() {
		...
		bookTitleColumn.setCellValueFactory(cellData -> {
			try {
				...
				BookValidator.validateNumericId(cellData.getValue().getBookId(), "Book ID");
				
		        Map<String, Object> criteria = new HashMap<>();
		        criteria.put("bookId", cellData.getValue().getBookId());
...
//				Book book = bookService.getBookById(cellData.getValue().getBookId());
		...
		Label noContentLabel = new Label("No Issued records found in the library.");
		noContentLabel.setStyle("-fx-text-fill: #999999; -fx-font-size: 16px; -fx-alignment: center;");
		issuedRecordsTableView.setPlaceholder(noContentLabel);
