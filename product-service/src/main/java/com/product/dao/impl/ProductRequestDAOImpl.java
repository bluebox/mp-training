...
public class ProductRequestDAOImpl implements ProductRequestDAO {
	...
	public ProductRequest createProductRequest(ProductRequest requestObj)
			throws ProductRequestDatabaseOperationException {
		...
		try {
			...
			if (recordInserted > 0) {
				...
				throw new ProductRequestDatabaseOperationException(
						"Failed to insert product request, no rows affected");
			}
		} catch (DataAccessException e) {
			log.error(e.getMessage(), e);
			throw new ProductRequestDatabaseOperationException("Database error occurred while creating product request",
					e);
		}
	}
	...
	public List<ProductRequest> fetchByCriteria(SearchProductRequestCriteria criteria)
		...
		try {
			...
			log.error(e.getMessage(), e);
			throw new ProductRequestDatabaseOperationException("Error while fetching product requests from database",
					e);
		}
	}

	@Override
	public void updatePendingRequest(ProductRequest RequestToUpdate) throws ProductRequestDatabaseOperationException {
		...
		try {
			...
			throw new ProductRequestDatabaseOperationException("Database error while updating product request.", e);
		}
	}
