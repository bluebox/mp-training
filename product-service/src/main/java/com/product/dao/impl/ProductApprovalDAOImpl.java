...
public class ProductApprovalDAOImpl implements ProductApprovalDAO {
	...
	@Override
	public Integer updateRequest(List<Long> productRequestId, RequestStatus status, Long managerId) {
		String sqlQuery = "UPDATE product_requests "
				+ "SET Status = :status, ApprovedBy = :approvedBy, UpdatedAtDate = :updatedAt "
				+ "WHERE ProductRequestId = :id";

		LocalDateTime now = LocalDateTime.now();
		MapSqlParameterSource[] batchParams = productRequestId.stream()
				.map(id -> new MapSqlParameterSource().addValue("status", status.getCode())
						.addValue("approvedBy", managerId).addValue("updatedAt", now).addValue("id", id))
				.toArray(MapSqlParameterSource[]::new);
		int[] updateCounts = jdbcTemplate.batchUpdate(sqlQuery, batchParams);
		...
		if (totalUpdated == 0) {
			throw new RuntimeException("No request found with IDs " + productRequestId + " to update status.");
		}
