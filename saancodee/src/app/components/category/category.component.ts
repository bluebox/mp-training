...
export class CategoryComponent implements OnInit {
  ...
  constructor(private http: HttpClient, private fb: FormBuilder, private route:ActivatedRoute, private service: RegisterService, private router: Router) { 
  ...
  apply_filters() {
    ...
    console.log(search.value, difficulty_level.value, this.categoryId);
    let data = {
      'search': search.value,
      'diff': Number(difficulty_level.value),
      'tags': [this.categoryId]
    }
    this.service.apply_filters(data).subscribe((data) => {
      console.log(data);
      this.problems = data
      this.p = []
    for (let i = (this.currentPage - 1) * 3; i < ((this.currentPage - 1) * 3) + 3; i ++ ) {
