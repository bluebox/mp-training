...
public class HomeController {
    ...
    private void goToAddMember(ActionEvent event) {
        try {
            ...
            controller.setAddMode(); // tell it to behave as "Add"
    ...
    private void goToUpdateMember(ActionEvent event) {
        try {
            ...
            ChoiceDialog<Member> dialog = new ChoiceDialog<Member>(members.get(0), members);
            ...
            dialog.showAndWait().ifPresent(selected -> {
                try {
                    ...
                    MemberController controller = loader.getController();
                    controller.setUpdateMode(selected); // tell it to behave as "Update"
    ...
    private void goToViewBooks() {
        try {
            Parent viewBooksRoot = FXMLLoader.load(getClass().getResource("/com/library/ui/ViewBooks.fxml"));
            Stage stage = new Stage();
            stage.setTitle("All Books");
            stage.setScene(new Scene(viewBooksRoot, 600, 400));
            stage.show();
        } catch (Exception e) {
            e.printStackTrace();
        }
