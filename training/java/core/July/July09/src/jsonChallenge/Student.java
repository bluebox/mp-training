...
public class Student {
	...
	public Student(int studentId, String countryCode, int yearEnrolled, int ageEnrolled, String gender,
	...
	public void addCourse(Course course, LocalDate enrollDate) {
		...
		if (!engagementMap.containsKey(course.getCourseCode())) {

			engagementMap.put(course.getCourseCode(),
					new CourseEngagement(course, enrollDate, "Online", new Random().nextInt(1, 30), enrollDate));
...
//	}
	...
	public String toString() {
		var engagementData = engagementMap.values().stream().map(CourseEngagement::toString)
		...
		return new StringJoiner(", ","{","}") 
				...
				.add("\"engagementMap\":" + engagementData)
				.toString();
	}
	
	public static Student getRandomStudent(List<Course> courses) {
		Random random = new Random();
		Student student = new Student(id++, ("+".concat(random.nextInt(50, 500) + "")), random.nextInt(2020, 2026),
				random.nextInt(1, 80), random.nextBoolean() ? "Male" : "Female", random.nextBoolean());
		for (Course course : courses) {
			student.addCourse(course);
			student.watchLecture(course.getCourseCode(), random.nextInt(5, course.getLectureCount()),
					random.nextInt(2020, 2030), random.nextInt(1, 12));
		}
		return student;
	}
}
