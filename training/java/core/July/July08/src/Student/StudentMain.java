...
public class StudentMain {
	public static void main(String[] args) {
		...
		var students = Stream.iterate(1,i->i+1).limit(10000).map(s-> Student.getRandomStudent(courses.subList(0, new Random().nextInt(0,4)))).collect(Collectors.toList());
		var starts= students.stream().mapToDouble(s->s.getPercentComplete()).summaryStatistics();
		...
		var groupByNumberCourse = students.stream().collect(Collectors.groupingBy(s->s.getEngagementMap().size(),Collectors.counting()));
		for(Entry<Integer, Long> count :groupByNumberCourse.entrySet()) {
			System.out.println("Number Courses "+count.getKey()+" Number Sudunts Enrolled: "+count.getValue());
		}
