				...
				if (empty) {
					...
					if (book != null) {
						toggleAvailabilityButton
								.setText(book.getAvailability() == BookAvailability.AVAILABLE ? "Mark Issued"
										: "Mark Available");
	...
	private void handleDeleteSelectedBatch() {
		...
		if (result.isPresent() && result.get() == ButtonType.OK) {
			try {
				...
				messageLabel.setTextFill(javafx.scene.paint.Color.GREEN);
				messageLabel.setText(successCount + " out of " + bookIdsToDelete + " books deleted successfully.");
				loadBooks();
			} catch (LibraryException e) {
				messageLabel.setTextFill(javafx.scene.paint.Color.RED);
				messageLabel.setText(e.getMessage());
