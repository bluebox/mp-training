...
public class IssueRecordDAO {
    ...
    public List<IssueRecord> getAllRecords() {
        return jdbcTemplate.query("SELECT * FROM issue_records",
    ...
    public IssueRecord getRecordById(int id) {
        return jdbcTemplate.queryForObject("SELECT * FROM issue_records WHERE issueId=?",
                new Object[]{id},
                (rs, rowNum) -> new IssueRecord(
                        rs.getInt("issueId"),
                        rs.getInt("bookId"),
    ...
    public int issueBook(IssueRecord record) {
        ...
        if ("I".equals(availability)) {
            // Book already issued
            return -1;  // special code for "already issued"
        }
