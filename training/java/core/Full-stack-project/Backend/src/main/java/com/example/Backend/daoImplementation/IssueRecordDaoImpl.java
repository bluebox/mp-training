...
public class IssueRecordDaoImpl implements IssueRecordDao {
	...
	public List<IssueRecord> getAllIssues() {
		String sql = "select issue_id,book_id,member_id,status,issue_date,return_date from issue_records";
		return jdbcTemplate.query(sql, issueRowMapper);
	}

	@Override
	public IssueRecord findActiveIssue(int memberId, int bookId) {
		String sql = "select issue_id,book_id,member_id,status,issue_date,return_date from issue_records where member_id=? and status=? and book_id=?";
		List<IssueRecord> list = jdbcTemplate.query(sql, issueRowMapper, memberId,
				IssueRecordStatus.ISSUED.getStringValue(), bookId);
		return list.isEmpty() ? null : list.get(0);
	}

	@Override
	public List<Book> getAvailableBooks() {
		String sql = "select issue_id,book_id,member_id,status,issue_date,return_date from books b where b.book_id not in (select book_id from issue_records where status = ?)";
		return jdbcTemplate.query(sql, (rs, rowNum) -> {
			...
			return book;
		}, BookStatus.ACTIVE.getStringValue());
	}

	@Override
	public List<Book> getIssuedBooksForMember(int memberId) {
		String sql = "select b.book_id,b.title,b.author,b.category,b.availability,b.status from books b join issue_records i on b.book_id = i.book_id where i.member_id = ? and i.status = ?";
		return jdbcTemplate.query(sql, (rs, rowNum) -> {
			Book book = new Book();
			book.setBookId(rs.getInt("book_id"));
			book.setTitle(rs.getString("title"));
