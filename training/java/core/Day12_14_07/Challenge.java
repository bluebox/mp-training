...
public class Challenge {
	...
	private static int addOrder(Connection conn, String[] items) throws SQLException {
		...
		try (Statement statement = conn.createStatement()) {
			...
			if (count != items.length) {
				orderId = -1;
				System.out.println("Mismatch in items inserted. Rolling back transaction.");
				conn.rollback();
			} else {
				conn.commit();
			}
	...
	private static void deleteOrder(Connection conn, int orderId) throws SQLException {
		String deleteSQL = "DELETE FROM %s WHERE order_id=%d";
		String childQuery = deleteSQL.formatted("storefront.order_details", orderId);
		String parentQuery = deleteSQL.formatted("storefront.order", orderId);

		try (Statement statement = conn.createStatement()) {
			conn.setAutoCommit(false);

			int deletedItems = statement.executeUpdate(childQuery);
			System.out.printf("Deleted %d records from order_details%n", deletedItems);

			int deletedOrder = statement.executeUpdate(parentQuery);
			if (deletedOrder == 1) {
				conn.commit();
				System.out.printf("Order %d successfully deleted%n", orderId);
			} else {
				conn.rollback();
			}
		} catch (SQLException e) {
