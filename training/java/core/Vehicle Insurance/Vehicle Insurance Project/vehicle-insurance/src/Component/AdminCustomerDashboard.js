...
const AdminCustomerDashboard = () => {
  ...
  const fetchCustomers = () => {
    axios.get("http://localhost:8000/customer/showAll")
  ...
  const submitForm = e => {
    e.preventDefault();
    if (editingId) {
      axios.put("http://localhost:8000/customer/update", { ...formData, customerId: editingId })
        .then(() => {
          fetchCustomers();
          resetForm();
        })
      ...
      axios.post("http://localhost:8000/customer/add", formData)
        .then(() => {
          fetchCustomers();
          resetForm();
        })
  ...
  const updateStatus = (id, status) => {
    axios.put(`http://localhost:8000/customer/updateStatus?customerId=${id}&status=${status}`)
      .then(() => fetchCustomers())
      .catch(err => console.error(err));
  };

  const deleteCustomer = id => {
    axios.put(`http://localhost:8000/customer/delete?customerId=${id}`)
      .then(() => fetchCustomers())
      .catch(err => console.error(err));
  };
