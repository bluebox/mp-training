...
//		}
	...
	public Policy getPolicyByVehicleId(int vehicleId) throws Exception {
		if(jdbcTemplate.queryForObject("select count(*) from policy where  vehicle_id=? and policy_status='A'",Integer.class,vehicleId)==0) {
			return new Policy();
		}
		return jdbcTemplate.queryForObject("select * from policy where  vehicle_id=? and policy_status='A'",new PolicyRowMapper(),vehicleId);
	}
	public Policy getPolicyByVehicle(String regNum){
		if(jdbcTemplate.queryForObject("select count(*) from policy p,vehicles v where p.vehicle_id=v.vehicle_id and v.reg_num=? and v.status='A' and p.policy_status='A'", Integer.class,regNum)==0) {
			return new Policy();
		}
		return jdbcTemplate.queryForObject("select p.* from policy p,vehicles v where p.vehicle_id=v.vehicle_id and v.reg_num=? and v.status='A' and p.policy_status='A'",new PolicyRowMapper(),regNum);
	}
	...
	public ArrayList<Object> getPolicyReport(int policyId) {
		if(jdbcTemplate.queryForObject("select count(*) from policy where policy_id=?", Integer.class,policyId)<=0) {
