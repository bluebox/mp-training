...
public class VehicleDao {
	...
	public String addVehicle(Vehicle vehicle) throws SQLException {
		...
		int rowsAffected=jdbcTemplate.update(vehicleAddSql,vehicle.getChasisNum(),vehicle.getRegNum(),vehicle.getVehicleModel(),
				vehicle.getPurchaseDate(),vehicle.getVehicleUpdatedOn(),vehicle.getVehicleUpdatedBy(),vehicle.getCustomerId(),vehicle.getCreatedBy(),'A');
		if(rowsAffected==0) {
...
//	}
//	
	public String updateVehicleNumber(int regNum,String updatedBy,LocalDateTime updatedOn,int vehicleId) throws SQLException {
		String sql="Update vehicles set reg_num=?,vehicle_updated_on=?,vehicle_updated_by=? where reg_num like temp% and vehicleId=?";
	...
	public Vehicle getVehicleById(int vehicleId) throws Exception {
		...
		try {
			vehicle=jdbcTemplate.queryForObject(sql, Vehicle.class, vehicleId);
		}
		catch(Exception e) {
	...
	public List<Vehicle> getAllVehicles() throws SQLException {
		...
		return jdbcTemplate.queryForList(sql,Vehicle.class);	
	}
}
