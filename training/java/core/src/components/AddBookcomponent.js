...
function AddBookComponent({token}) {
...
const [availability,setavailability]=useState('');
  ...
  var response;
        try{
             response=await axios.post("http://localhost:8095/Book/addbook", {
  bookid:1,
  title:title,
  author:author,
  category:category,
...
alert("book added successfully");
	...
	<input type="text" name="title" id="title" value={title} onChange={handlechange} required/></div>
    <div><label htmlFor="author">Author:</label>
	<input type="text" name="author" id="author" value={author} onChange={handlechange} pattern="^[a-zA-Z\s]+$" title="name should not contain numbers" required/></div>
    <div><label htmlFor="category">Category:</label>
	<input type="text" name="category" id="category" value={category} onChange={handlechange} required/></div>
    <div>
