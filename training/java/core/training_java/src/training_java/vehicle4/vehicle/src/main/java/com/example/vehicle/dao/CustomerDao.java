...
public class CustomerDao {
	...
	public int addCustomer(Customer customer) throws SQLException {
		String custAddSql = "Insert into customers(name,email,contact,gender,age,occupation,income,address,status,customer_updated_on,customer_updated_by,created_by)"				+ "values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		int rowsAffected = jdbcTemplate.update(custAddSql, customer.getName(), customer.getEmail(), customer.getContact(),customer.getGender().getVal(), customer.getAge(), customer.getOccupation(), customer.getIncome(),	customer.getAddress(), "A",LocalDateTime.now(), customer.getCreatedBy(),customer.getCreatedBy());
		if (rowsAffected == 0) {
			return -1;
		}
		else {
	...
	public String deleteCustomerById(int customerId) throws Exception {
		...
		if(rowsAffected==0) {
			return "There is No Active Customer with this  customerId";
		}
		else {
