...
public class PolicyDao {
	...
	public String updateStatus(int policyId) {
		...
		char vehicleStatus=jdbcTemplate.queryForObject("select v.status from vehicles v,policy p where v.vehicle_id=p.vehicle_id and p.policy_id=?", Character.class,policyId);
		if(vehicleStatus=='I') {
			return "There is No Active vehicle for this policy";
		}
	...
	public String updateStatus(int policyid, char policyStatus, String approvedBy) {
		...
		int rowsEffected = jdbcTemplate.update("update policy set policy_status=?,approved_by=? where policy_id=?",
				String.valueOf(policyStatus), approvedBy, policyid);
		if (rowsEffected > 0) {
			return "Status is updated";
		} else {
			return "Failed to update status";
