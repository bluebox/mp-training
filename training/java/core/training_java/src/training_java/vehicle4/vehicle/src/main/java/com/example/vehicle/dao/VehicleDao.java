...
public class VehicleDao {
	...
	public String addVehicle(Vehicle vehicle) throws SQLException {
		...
		int rowsAffected = jdbcTemplate.update(vehicleAddSql, vehicle.getChasisNum(), vehicle.getRegNum(),
				vehicle.getVehicleModel(), LocalDateTime.now(), LocalDateTime.now(), vehicle.getCreatedBy(),
				vehicle.getCustomerId(), vehicle.getCreatedBy(), "A");
		if (rowsAffected == 0) {
...
//	public String updateVehicle(Vehicle vehicle) throws SQLException {
...
//	
	...
	public Vehicle getVehicleById(int vehicleId) throws Exception {
		String sql = "Select vehicle_id,chasis_no,reg_num,vehicle_model,purchase_date,vehicle_updated_on,vehicle_updated_by,customer_id,created_by,status from vehicles where vehicle_id=?";
	...
	public Vehicle getVehicleByRegNum(String regNum) throws Exception {
		String sql = "Select vehicle_id,chasis_no,reg_num,vehicle_model,purchase_date,vehicle_updated_on,vehicle_updated_by,customer_id,created_by,status from vehicles where reg_num=?";
		Vehicle vehicle = null;
		try {
			vehicle = jdbcTemplate.queryForObject(sql, new VehicleRowMapper(), regNum);
		} catch (Exception e) {
			throw new Exception("No Vehicle found with Registration Number: " + regNum);
		}
		return vehicle;
	}
	...
	public String deleteVehicleById(int vehicleId) throws Exception {
		...
		String policySql = "Update policy set policy_status='I' where vehicle_id=? and status='A'";
	...
	public List<Vehicle> getAllVehiclesByCustomer(int customerId) {
		String sql = "Select vehicle_id,chasis_no,reg_num,vehicle_model,purchase_date,vehicle_updated_on,vehicle_updated_by,customer_id,created_by,status from vehicles where customer_id=? and status='A'";
		return jdbcTemplate.query(sql, new VehicleRowMapper(), customerId);

	}

	public List<Vehicle> getAllVehicles() {
		String sql = "Select vehicle_id,chasis_no,reg_num,vehicle_model,purchase_date,vehicle_updated_on,vehicle_updated_by,customer_id,created_by,status from vehicles";
