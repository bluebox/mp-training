...
public class UserDao {
	...
	public String addUser(User u) throws SQLException {
		if (jdbcTemplate.queryForObject("select count(username) from users where customer_id=?", Integer.class,
				u.getCustomerId()) > 0) {
			return "User is already existed";
		}
		int rowEffected = jdbcTemplate.update(
				"insert into users(username,password,password_updated_on,password_updated_by,customer_id) values(?,?,?,?,?)",
				u.getUsername(), u.getPassword(), LocalDateTime.now(), u.getPasswordUpdatedBy(), u.getCustomerId());
		if (rowEffected > 0) {
	...
	public String updatePassword(String username, String password, String passwordUpdatedBy) throws SQLException {
		int rowEffected = jdbcTemplate.update(
				"update users set password=?,password_updated_on=?,password_updated_by=? where username=?", password,
				LocalDateTime.now(), passwordUpdatedBy, username);
		if (rowEffected > 0) {
