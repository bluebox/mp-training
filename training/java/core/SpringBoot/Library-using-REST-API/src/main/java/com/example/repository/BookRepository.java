...
public class BookRepository{
	...
	public String updateById(long bookId,String title,String author,String category,char status,char availability) {
		int x=jdbcTemplate.update("update books set title=?, author=?, category=?, status=?, availabilty=? where bookId=?",title,author,category,"A","A",bookId);
		if(x>0) {
			return "updated";
		}
		return "not updated";
	...
	public int changeStatus(long bookId) {
		...
		return jdbcTemplate.update("update books set status=?,availabilty=? where bookId=?",(String.valueOf(l.get("Status")).charAt(0)=='A'?"I":"A"),(String.valueOf(l.get("Status")).charAt(0)=='A'?"I":"A"),bookId);
	}
}
