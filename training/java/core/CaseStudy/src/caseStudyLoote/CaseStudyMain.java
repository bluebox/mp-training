...
public static void main(String [] args) throws IOException {
	...
	List<EmployeeWorkLog> logs = readExcel("C:\\Users\\gopin\\OneDrive\\Desktop\\medplus\\mp-training\\training\\java\\core\\CaseStudy\\src\\caseStudyLoote\\sample_5_employees_timesheet.xlsx");
    ...
    static List<EmployeeWorkLog> readExcel(String path) throws IOException {
        ...
        try (InputStream fis = Files.newInputStream(Paths.get(path));
            ...
            for (int i = 1; i <= sheet.getLastRowNum(); i++) {
                ...
                String remarks = row.getCell(3).getStringCellValue();
            ...
            .collect(Collectors.groupingBy(EmployeeWorkLog::getEmployeeId,
                Collectors.groupingBy(log -> YearMonth.from(log.getDate()),
                    Collectors.collectingAndThen(
                        Collectors.toList(),
                        monthlyLogs -> {
                            double totalHours = monthlyLogs.stream().mapToDouble(EmployeeWorkLog::getHoursWorked).sum();
