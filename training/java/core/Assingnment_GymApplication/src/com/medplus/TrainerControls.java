...
public abstract class TrainerControls {
		...
		while (true) {
			...
			if (choosingNumber == 1) {
				...
				if (tempMembers.size() == 0) {
					System.out.println("No Members availabe !!!");
				} else {
	...
	public void addMember() {
		...
		Member temp = new Member(id, Input.getValidatedName(), Input.calculateAge(Input.getValidatedDOB()),
				LocalDate.now(), Input.getValidatedMembershipPlan());
