...
public class OrdersServiceImpl implements OrdersService {
	...
	public OrdersServiceImpl(OrdersDao ordersDao, OrderDetailsDao orderDetailsDao,
			OrderDetailsService orderDetailsService, WaitersService waitersService, CustomerService customerService,
	...
	@Override
	@Transactional
	public Orders addOrder(String name, String phone, int waiterId, double totalPrice,
			List<OrderDetails> orderDetailsList) {
		if (name == null || name.trim().isEmpty()) {
			throw new RestaurantOperationException("Customer name cannot be null or empty");
		}
		if (phone == null || !phone.matches("^[6-9]\\d{9}$")) {
			throw new RestaurantOperationException("Mobile number must be 10 digits and start with 6-9");
		}
		if (waiterId <= 0) {
			throw new RestaurantOperationException("Invalid waiter ID");
		}
		if (totalPrice <= 0) {
			throw new RestaurantOperationException("Total price must be greater than zero");
		}
		if (orderDetailsList == null || orderDetailsList.isEmpty()) {
			throw new RestaurantOperationException("Order details cannot be empty");
		}
		if (orderDetailsList == null || orderDetailsList.isEmpty()) {
			throw new RestaurantOperationException("Order details cannot be empty");
		}
	...
	public void updateStatus(int id, String status) {
		if (id <= 0) {
			throw new RestaurantOperationException("Invalid order ID");
		}
		if (status == null || status.trim().isEmpty()) {
			throw new RestaurantOperationException("Status cannot be null or empty");
		}
		...
		if (!order.getStatus().equals(OrderStatus.PENDING)) {
			throw new RestaurantOperationException("Cannot update status. Order with ID " + order.getOrderId()
					+ " is already " + order.getStatus().getStatus());
		}
...
//		}
	...
	public List<Orders> getOrdersByCategory(String category) {
		if (category == null || category.trim().isEmpty()) {
			throw new RestaurantOperationException("Category cannot be null or empty");
		}
