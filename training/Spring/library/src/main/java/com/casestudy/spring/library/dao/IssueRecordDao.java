...
public class IssueRecordDao implements IssueRecordDaoModel {
	...
	public void returnBook(IssueRecord issueRecord) throws SQLException {
		...
		String isReturned = "SELECT issueDate FROM IssueRecords WHERE bookId = ? AND memberId = ? AND status = 'I'";
		PreparedStatement ps1 = conn.prepareStatement(isReturned);

		ps1.setInt(1, issueRecord.getBookId());
		ps1.setInt(2, issueRecord.getMemberId());
