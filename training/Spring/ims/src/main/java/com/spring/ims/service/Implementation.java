...
public class Implementation {
	...
	public boolean creationOfOrder(FullOrder fullOrder) {
		...
		for(int i=0;i<listOfProducts.size();i++) {
			orderCost = orderCost + (listOfProducts.get(i).getProductQuantity() * this.costOfProduct(listOfProducts.get(i)));
		}
	...
	public boolean approval(Orders order) {
		return ordersRepository.approval(order);
	}
