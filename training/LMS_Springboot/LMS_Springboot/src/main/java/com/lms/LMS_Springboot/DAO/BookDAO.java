...
public class BookDAO {
    ...
    public List<Book> getAllBooks() {
        return jdbcTemplate.query("SELECT bookId,title,author,category,status,availability FROM books",
                (rs, rowNum) -> new Book(
                        rs.getInt("bookId"),
                        rs.getString("title"),
                        rs.getString("author"),
    ...
    public Book getBookById(int id) {
        return jdbcTemplate.queryForObject("SELECT bookId,title,author,category,status,availability FROM books WHERE bookId=?",
                new Object[]{id},
                (rs, rowNum) -> new Book(
                        rs.getInt("bookId"),
                        rs.getString("title"),
