...
export default function UserDashboard() {
  ...
  const fetchCustomerId = () => {
    axios.get(`http://localhost:8080/user/show?username=${username}`, { withCredentials: true })
      .then(res => {
  ...
  const fetchCustomerDetails = (id) => {
    axios.get(`http://localhost:8080/customer/show?customerId=${id}`, { withCredentials: true })
      .then(res => {
  ...
  const fetchVehicles = (id) => {
    axios.get(`http://localhost:8080/vehicle/showByCustomerId?customerId=${id}`, { withCredentials: true })
      .then(res => setVehicles(res.data))
      .catch(err => console.error(err));
  };

  const fetchPoliciesByUser = () => {
    axios.get(`http://localhost:8080/policy/showByUser?username=${username}`, { withCredentials: true })
      .then(res => setPolicies(res.data))
      .catch(err => console.error(err));
  };

  const fetchClaimsByUser = () => {
    axios.get(`http://localhost:8080/claim/claimByUser?username=${username}`, { withCredentials: true })
      .then(res => setClaims(res.data))
      .catch(err => console.error(err));
  };

  const updateVehicleReg = () => {
    axios.put(`http://localhost:8080/vehicle/update?vehicleId=${selectedVehicleId}&regNum=${newRegNum}&updatedBy=${username}`,{}, { withCredentials: true })
      .then((response) => {
        alert(response.data);
  ...
  const submitPolicyRequest = () => {
    axios.post(`http://localhost:8080/policy/add`, {
  ...
  const submitClaimRequest = () => {
    axios.post(`http://localhost:8080/claim/add`, {
  ...
  const payPolicyDue = (policyId) => {
    axios.put(`http://localhost:8080/policy/payDue?policyId=${policyId}`, {}, { withCredentials: true })
      .then((response) => {
        alert(response.data);
        fetchPoliciesByUser();
      })
  ...
  const updateCustomerDetails = () => {
    axios.put(`http://localhost:8080/customer/update`, { ...customerForm, customerUpdatedBy: username }, { withCredentials: true })
      .then((response) => {
        alert(response.data);
  ...
  const updatePassword = () => {
    axios.put(`http://localhost:8080/user/updatePassword?username=${username}&password=${newPassword}`, {}, { withCredentials: true })
      .then((response) => {
        alert(response.data);
        setNewPassword('');
      })
  ...
  axios.get(`http://localhost:8080/policy/report?policyId=${policyId}`, { withCredentials: true })
    .then(res => {
      const doc = new jsPDF();
...
const generateClaimReport = (claimId) => {
  axios.get(`http://localhost:8080/claim/claimReports?claimId=${claimId}`, { withCredentials: true })
    .then(res => {
      const doc = new jsPDF();
