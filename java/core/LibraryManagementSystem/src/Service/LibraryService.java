...
public class LibraryService {
	...
	public void issueBook(int bookId, int memberId) throws Exception {
		...
		if (book == null || book.getAvailability() == 'I') {
	...
	//View &Streams
	 public List<Book> viewAllBooks() throws Exception {
	    ...
	    public List<IssueRecord> viewIssuedRecords() throws Exception {
	        ...
	        if ( records.isEmpty()) throw new Exception("No members found");
	        return  records;
	    }
	    ...
	    public List<Member> getMembersWithIssuedBooks() throws SQLException {
	        ...
	        return memberDAO.getAllMembers().stream()
	            .filter(member -> issuedMemberIds.contains(member.getMemberId()))
	            .collect(Collectors.toList());
	    }
...
//		lib.viewAllBooks().forEach(b->System.out.println(b));
//		lib.viewAllMembers().forEach(m->System.out.println(m));
//		System.out.println(lib.countBooksByCategory());
//		
//	}
}
