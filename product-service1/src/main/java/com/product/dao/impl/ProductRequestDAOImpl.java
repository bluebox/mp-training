...
public class ProductRequestDAOImpl implements ProductRequestDAO {
	...
	@Override
	public ProductRequest createProductRequest(ProductRequest requestObj) {
		...
		try {
			...
			if (recordInserted > 0) {
				...
				throw new DatabaseOperationException("Failed to insert product request, no rows affected");
			}
		} catch (DataAccessException e) {
			throw new DatabaseOperationException("Database error occurred while creating product request", e);
		}
	}
	...
	public List<ProductRequest> fetchByCriteria(SearchProductRequestCriteria criteria) {
		StringBuilder sql = new StringBuilder(
				"SELECT ProductRequestId, ProductName, Description, Price, Quantity, RequestedBy, Status, ApprovedBy, CreatedAtDate, UpdatedAtDate FROM product_requests WHERE 1=1");
		...
		sql.append(" AND (:requestIdsFlag = 0 OR ProductRequestId IN (:requestIds))")
				.append(" AND (:productNamesFlag = 0 OR ProductName IN (:productNames))")
				.append(" AND (:userIdFlag = 0 OR RequestedBy = :userId)")
				.append(" AND (:statusFlag = 0 OR Status = :status)");
		...
		try {
			...
			throw new DatabaseOperationException("Error while fetching product requests from database", e);
		}
	}

	@Override
	public void updatePendingRequest(ProductRequest RequestToUpdate) {
		String sql = "UPDATE product_requests "
				+ "SET ProductName = :name, Description = :description, Price = :price, Quantity = :qty, UpdatedAtDate = :updatedAt "
				+ "WHERE ProductRequestId = :requestId AND Status = :pendingStatus";
		...
		try {
			...
			throw new DatabaseOperationException("Database error while updating product request.", e);
		}
	}
